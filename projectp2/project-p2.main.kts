...
fun testTaggedFlashCards() {
    testSame(flashCard1.isTagged("easy"), true, "tag is present")
    testSame(flashCard1.isTagged("hard"), false, "tag is absent")
    testSame(flashCard1.fileFormat(), "What is the capital of France?|Paris|geography,easy", "file format")
}
...
// parses a string into a tagged flash card
fun stringToTaggedFlashCard(string: String): TaggedFlashCard {
    val parts = string.split(sepCard)
    val tags = parts[2].split(sepTag)
    return TaggedFlashCard(parts[0], parts[1], tags)
}
...
// reads a file of tagged flash cards which are seperated by lines and parses them into cards
fun readTaggedFlashCardsFile(path: String): List<TaggedFlashCard> {
    val lines = fileReadAsList(path)
    return lines.map(::stringToTaggedFlashCard)
}

val exampleFlashCard1 =
    TaggedFlashCard(
        front = "c",
        back = "3",
...
fun testReadTaggedFlashCardsFile() {
    testSame(
        ...
        listOf(exampleFlashCard1, exampleFlashCard2),
        "read tagged cards",
    )

    testSame(
        readTaggedFlashCardsFile("invalid.txt"),
        listOf(),
        "read tagged cards (does not exist)",
    )
}
...
// Represents a deck of tagged flash cards
class TFCListDeck(
    ...
    override fun next(correct: Boolean): IDeck {
        return when (state) {
            DeckState.ANSWER -> {
                if (correct) {
                    if (cards.size == 0) {
                        TFCListDeck(cards, DeckState.EXHAUSTED)
                    } else if (cards.size == 1) {
                        TFCListDeck(cards.drop(1), DeckState.EXHAUSTED)
                    } else {
                        TFCListDeck(cards.drop(1), DeckState.QUESTION)
                    }
                } else {
                    TFCListDeck(cards.drop(1) + cards[0], DeckState.QUESTION)
                }
            }
            else -> this
        }
...
val testListOfCards = listOf(flashCard1, flashCard2, flashCard3)
val testTFCListDeck = TFCListDeck(testListOfCards)
val emptyTFCListDeck = TFCListDeck(listOf())

@EnabledTest
fun testTFCListDeck() {
    testSame(testTFCListDeck.getState(), DeckState.QUESTION, "initial state")
    testSame(testTFCListDeck.getText(), "What is the capital of France?", "initial text")
    testSame(testTFCListDeck.getSize(), 3, "initial size")
    testSame(testTFCListDeck.flip().getState(), DeckState.ANSWER, "flip state")
    testSame(testTFCListDeck.flip().getText(), "Paris", "flip text")
    testSame(testTFCListDeck.flip().getSize(), 3, "flip size")

    val nextDeckCorrect = testTFCListDeck.flip().next(true)
    val nextDeckIncorrect = testTFCListDeck.flip().next(false)
    val nextDeckEmpty = emptyTFCListDeck.flip().next(true)

    testSame(nextDeckCorrect.getState(), DeckState.QUESTION, "next state")
    testSame(nextDeckCorrect.getText(), "What is the square root of 16?", "next text")
    testSame(nextDeckCorrect.flip().getText(), "4", "next text after flip")
    testSame(nextDeckCorrect.getSize(), 2, "next size when correct")
    testSame(nextDeckIncorrect.getSize(), 3, "next size when incorrect")

    testSame(nextDeckEmpty.getState(), DeckState.EXHAUSTED, "empty state")
}
...
// Represents a deck of perfect squares- without actually generating or storing flash cards.
// Instead, it accepts a list of numbers and emulates having cards built from those numbers.
class PerfectSquaresDeck(private val numbers: List<Int>, private val state: DeckState = DeckState.QUESTION) : IDeck {
    // The state of the deck
    override fun getState(): DeckState {
        return state
    }
    ...
    override fun getText(): String? {
        return when (state) {
            DeckState.EXHAUSTED -> null
            DeckState.QUESTION -> ("${numbers[0]}^2 = ?")
            DeckState.ANSWER -> (numbers[0] * numbers[0]).toString()
        }
    }
    ...
    override fun flip(): IDeck {
        return when (state) {
            DeckState.QUESTION -> PerfectSquaresDeck(numbers, DeckState.ANSWER)
            else -> this
        }
    }
    ...
    override fun next(correct: Boolean): IDeck {
        return when (state) {
            DeckState.ANSWER -> {
                if (correct) {
                    if (numbers.size == 0) {
                        PerfectSquaresDeck(numbers, DeckState.EXHAUSTED)
                    } else {
                        PerfectSquaresDeck(numbers.drop(1), DeckState.QUESTION)
                    }
                } else {
                    PerfectSquaresDeck(numbers.drop(1) + numbers[0], DeckState.QUESTION)
                }
            }
            else -> this
        }
...
fun testPerfectSquaresDeck() {
    val deck = PerfectSquaresDeck(listOf(1, 2, 3))
    val emptyDeck = PerfectSquaresDeck(listOf())

    testSame(deck.getState(), DeckState.QUESTION, "initial state")
    testSame(deck.getText(), "1^2 = ?", "initial text")
    testSame(deck.getSize(), 3, "initial size")
    testSame(deck.flip().getState(), DeckState.ANSWER, "flip state")
    testSame(deck.flip().getText(), "1", "flip text")
    testSame(deck.flip().getSize(), 3, "flip size")

    val nextDeckCorrect = deck.flip().next(true)
    val nextDeckIncorrect = deck.flip().next(false)
    val nextDeckEmpty = emptyDeck.flip().next(true)

    testSame(nextDeckCorrect.getState(), DeckState.QUESTION, "next state")
    testSame(nextDeckCorrect.getText(), "2^2 = ?", "next text")
    testSame(nextDeckCorrect.flip().getText(), "4", "next text after flip")
    testSame(nextDeckCorrect.getSize(), 2, "next size when correct")
    testSame(nextDeckIncorrect.getSize(), 3, "next size when incorrect")
...
// Takes in a string and a range of valid indices and returns the string as a list index if it is valid (or -1 if 0 was entered)
fun keepIfValid(
    typedInput: String,
    ...
    if (!isAnInteger(typedInput)) {
        ...
        val typedInputIndex = typedInput.toInt() - 1
        return when {
            typedInputIndex == -1 -> typedInputIndex
            typedInputIndex !in validIndices -> -2
            else -> typedInputIndex
        }
    }
}
...
fun keepIfValidTest() {
    ...
    testSame(keepIfValid("4", 0..2), -2, "exceeds range")
    testSame(keepIfValid("0", 0..2), -1, "0 (quit)")
    testSame(keepIfValid("a", 0..2), -2, "invalid input")
    testSame(keepIfValid("", 0..2), -2, "empty string")
}

// Takes in a list of strings and returns a string of the list items with new lines between each item
fun choicesToText(choices: List<String>): String {
    val numberedChoices = choices.mapIndexed { index, choice -> "${index + 1}. $choice" }
    return linesToString(numberedChoices + "" + menuPrompt)
}

// This test does not pass. Honestly, I have no idea why. I've even tried using a text compare tool
// to see where the difference lies but whatever it is does not persist when copied to my clipboard.
// I've tried switching up the number of new lines and where they are but I've been unsuccessful.
@EnabledTest
fun testChoicesToText() {
    testSame(
        choicesToText(listOf("a", "b", "c")),
        linesToString(listOf("1. a", "2. b", "3. c", "", menuPrompt)),
        "choices to text",
    )
}
...
//     // TODO: nothing, but understand this :)
    fun isTerminalState(state: Int): Boolean {
        return state == -1 || state in options.indices
    }
...
// returns the top k items in a list based on their evaluation function
fun <T> topK(
    items: List<T>,
    k: Int,
    evalFunc: EvaluationFunction<T>,
): List<T> {
    val scores = items.map { evalFunc(it) }
    val zipped = items.zip(scores)
    val sorted = zipped.sortedByDescending { it.second }
...
fun testTopK() {
    testSame(
        topK(listOf("a", "bb", "ccc"), 2) { it.length },
        listOf("ccc", "bb"),
        "top 2 by length",
    )
    testSame(
        topK(listOf("a", "bb", "ccc"), 4) { it.length },
        listOf("ccc", "bb", "a"),
        "top 4 by length",
    )
    testSame(
        topK(listOf(1, 2, 3), 2) { it },
        listOf(3, 2),
        "top 2 by value",
    )
}
...
// uses the datasetYN to predict the label
fun yesNoClassifier(s: String): ResultWithVotes<Boolean> {
    ...
    val exactMatch = datasetYN.any { it.example == lowerCaseInput }
    ...
    return when (exactMatch) {
        true -> ResultWithVotes(datasetYN.first { it.example == lowerCaseInput }.label, 3)
        false -> nnLabel(lowerCaseInput, datasetYN, ::levenshteinDistance, classifierK)
    }
}
...
// // uses the yesNoClassifier to predict the label
...
data class StudyState(val deck: IDeck, val numQuestions: Int, val numAttempts: Int = 0)

// Takes in a deck and a classifier and then studies the deck using the classifier
fun studyDeck2(
    deck: IDeck,
    ...
    fun transitionState(
        ...
        // returns a new study state based on the user's input and the current state
        // also uses the functions provided by the interface IDeck (those being flip and next)
        return when (state.deck.getState()) {
            DeckState.QUESTION -> {
                StudyState(state.deck.flip(), state.numQuestions, state.numAttempts)
            }
            else -> {
                val isCorrect = classifier(kbInput)
                StudyState(state.deck.next(isCorrect), state.numQuestions, state.numAttempts + 1)
            }
        }
    }

    fun isDone(state: StudyState): Boolean {
        return state.deck.getSize() == 0
    }

    fun terminalStateToText(state: StudyState): String {
        return "Questions: ${state.numQuestions}, Attempts: ${state.numAttempts}"
    }

    val reactConsoleOutput =
        reactConsole(
            initialState = StudyState(deck, deck.getSize()),
    ...
    return StudyDeckResult(reactConsoleOutput.numQuestions, reactConsoleOutput.numAttempts)
}
...
fun testStudyDeck2() {
    testSame(
        captureResults(
            { studyDeck2(easyFilteredDeck, ::isPositiveSimple) },
            "",
            "y",
            "",
            "y",
...
val easyFilteredDeck = TFCListDeck(testListOfCards.filter { it.isTagged("easy") })

// Chooses a deck and a classifier and then studies the deck using the classifier. Repeats until the user quits.
fun study2() {
    val decks =
        listOf(
            NamedMenuOption(TFCListDeck(readTaggedFlashCardsFile("capitals.txt")), "Capitals (loaded from file)"),
            NamedMenuOption(PerfectSquaresDeck(listOf(1, 4, 8)), "Perfect Squares"),
            NamedMenuOption(
                easyFilteredDeck,
                "Easy Flashcards (sorted)",
            ),
        )
    ...
    while (true) {
        val deck = chooseMenuOption(decks) ?: return

        val classifier = chooseMenuOption(classifierOptions) ?: return
...
fun testStudy2() {
    testSame(
        ...
        CapturedResult(
            Unit,
            "1. Capitals (loaded from file)",
            "2. Perfect Squares",
            "3. Easy Flashcards (sorted)",
            "",
            "$menuPrompt",
            "${menuChoicePrefix}Capitals (loaded from file)",
            "1. $optSimple",
            "2. $optML",
            "",
            "$menuPrompt",
    ...
    testSame(
        ...
        CapturedResult(
            ...
            "",
            "$menuPrompt",
            "${menuChoicePrefix}Perfect Squares",
            "1. $optSimple",
            "2. $optML",
            "",
            "$menuPrompt",
    ...
    testSame(
        ...
        CapturedResult(
            ...
            "",
            "$menuPrompt",
            "${menuChoicePrefix}Easy Flashcards (sorted)",
            "1. $optSimple",
            "2. $optML",
            "",
            "$menuPrompt",
